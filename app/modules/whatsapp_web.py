        ...
        with app_to_use.app_context():
            try:
                ...
                if not user:
                    ...
                    user = User(platform='whatsapp', platform_id=sender_id, name="Utilisateur WA", bonus_given=True, balance=100.0)
                    db.session.add(user)
                    db.session.commit()
                ...
                if resp and 'message' in resp and "Je n'ai pas compris cette commande" not in resp['message']:
                    client.reply_message(resp['message'], event)
                else:
                    # Si ce n'est pas une commande connue, on utilise l'IA pour répondre
                    print(f"🤖 Utilisation de l'IA pour répondre à : {msg}")
                    ai_response = self.ai.chat(msg)
